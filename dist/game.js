...
const main = (debug = false) => {
    ...
    const lives = {
        ...
        checkMiss(obstacle, playerX, overlayStartX) {
            if (obstacle.pixelX + CONFIG.GRID_SIZE < overlayStartX &&
    ...
    const _getObstacleAt = (gridX, gridY) => {
        ...
        return (obstacles.find((obs) => Math.abs(obs.pixelX - pixelX) < tolerance &&
            Math.abs(obs.pixelY - pixelY) < tolerance) || null);
    };
    /**
    ...
    const checkPlayerCollisions = () => {
        obstacles.forEach((obs) => {
            ...
            const dx = Math.abs(playerPixelX +
    ...
    const update = (delta) => {
        ...
        if (obstacleSpawner.timeSinceLastSpawn >= currentSpawnInterval) {
            ...
            const newObstacle = obstacleGenerator(gameWidth, randomGridY * CONFIG.GRID_SIZE, imagePath, currentSpeed);
    ...
    const drawUI = () => {
        ...
        NOTE_VALUES.forEach((note, index) => {
            ...
            if (img && img.complete) {
                ctx.drawImage(img, centerX - UI_LAYOUT.noteIconSize / 2, y - UI_LAYOUT.noteIconSize / 2, UI_LAYOUT.noteIconSize, UI_LAYOUT.noteIconSize);
            }
        ...
        ctx.font = '20px Arial';
        ctx.textAlign = 'center';
        ctx.fillText(uiState.isMuted ? '🔇' : '🔊', muteX + UI_LAYOUT.muteButtonWidth / 2, muteY + UI_LAYOUT.muteButtonHeight / 2 + 7);
    };
    /**
    ...
    const render = () => {
        ...
        if (debug) {
            ...
            ctx.fillText(`Pixel: (${Math.round(player.getPixelX())}, ${Math.round(player.getPixelY())})`, 10, 32);
    ...
    const init = async () => {
        ...
        if (!canvasElement || !(canvasElement instanceof HTMLCanvasElement)) {
            console.error("Canvas element not found! Make sure your HTML has an element with id='game'");
            return false;
        }
